...
## and in na pos we put 1, i.e. the pixeln NA is white
	...
	## match data and roi names
		...
		scaled <- sapply(x, function(x) (x-min(x,na.rm=TRUE))/(max(x, na.rm=TRUE)-min(x, na.rm=TRUE)), simplify=FALSE)
		pos.na <- sapply(scaled, function(x) which(is.na(x)), simplify=FALSE)
		scaled <- sapply(scaled, function(x) replace(x, which(is.na(x)), 0.5), simplify=FALSE)
		pos.pix.roi <- sapply(roi.data.ordered, function(x) which(x$pixels.in.roi$pip==1), simplify=FALSE)
		ratio <- dim(img)[1]/dim(img)[2]
		par(mar=c(rep(1,4)))
		plot(0, type = "n", xlim = c(0, 1), ylim = c(0, 1), axes = FALSE, ylab='', xlab='')
		rasterImage(img, xleft = 0, ybottom = 0, xright = 1, 
