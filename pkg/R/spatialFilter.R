...
function(spatial.list, filter=c('night','spline', 'max'), filter.options=NULL, ncores='all',
    ...
    fitting.list <- foreach(a=1:npixels, .packages=c('phenopix'), .combine=cbind) %dopar% {
                   if (!is.null(log.file)) {
    ...
    filtered.tmp <- phenopix::autoFilter(single.pixel, raw.dn=TRUE, dn=1:3, plot=FALSE, filter=filter, filter.options=filter.options)
        if (length(filter)!=1) {
